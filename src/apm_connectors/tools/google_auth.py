...
def load_credentials(
    ...
    On a headless deployment (no browser, no local `localhost` redirect
    to catch — e.g. the ECS Fargate deploy in `infra/aws/ecs-fargate/`)
    the interactive flow below can't run at all. For that case, set
    `GOOGLE_TOKEN_JSON` to the full contents of a `.google_token.json`
    generated by running this same interactive flow once, locally (see
    docs/running-locally.md); it takes precedence over `token_path` and
    is used as-is, refreshed in-memory via its `refresh_token` as
    needed, same as the file-backed cache.
    ...
    from_env = bool(settings.google_token_json)
    creds = None
    if from_env:
        creds = Credentials.from_authorized_user_info(json.loads(settings.google_token_json), scopes)
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            raise RuntimeError(
                "GOOGLE_TOKEN_JSON is set but its token is invalid/expired and has "
                "no refresh_token. Generate a fresh one with the local interactive "
                "flow and update it."
            )
        else:
